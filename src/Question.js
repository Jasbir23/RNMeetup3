...
export default class Question extends Component {
  ...
  _showOptions(obj) {
    return Object.keys(obj.options).map((item, index) => {
      return (
        <View
          key={index}
          style={{
        ...
        >
          ...
          >
            <Text
              style={{
                color: this.state.selectedOption === index ? "green" : "black",
                paddingHorizontal: 5
              }}
            >
              {item}. {obj.options[item].value}
            </Text>
          </TouchableOpacity>
        </View>
      );
            ...
            ? <View>
                {Object.keys(this.state.questions).map((item, index) => {
                  if (
                    ...
                    this.state.questions[item].id
                  ) {
                    console.log(item, "val");
                    return (
                      <View key={index} style={{ marginVertical: 10 }}>
                        <Text>
                          Q.{this.state.selectedQuestion + " " + item}
                        </Text>
